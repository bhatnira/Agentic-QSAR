"""A compact, CPU-friendly Message Passing Neural Network.

Implements a scikit-learn compatible estimator over :class:`MolGraph` inputs
(identical contract to the GCN plugin) using a mean-aggregation message passing
scheme followed by a sum readout.  Torch is optional: the plugin only becomes
available when torch is installed.
"""
...
class _MPNNLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, adj: torch.Tensor) -> torch.Tensor:
        messages = torch.bmm(adj, self.msg(x))  # neighbor aggregation (self-looped)
        deg = adj.sum(dim=-1, keepdim=True).clamp(min=1e-6)
        messages = messages / deg
        concat = torch.cat([x, messages], dim=-1)
        return F.relu(self.update(concat))


class _MPNNModule(nn.Module):
    def __init__(self, hidden: int = 64, n_out: int = 1, n_layers: int = 2) -> None:
        ...
        for _ in range(n_layers):
            self.layers.append(_MPNNLayer(in_dim, hidden))
            in_dim = hidden
...
class TorchMPNN:
    """sklearn-style estimator (`fit`/`predict`) wrapping the MPNN module."""
    ...
    def __init__(
        ...
        if not _TORCH_OK:
            raise PluginUnavailableError(
                "torch is not installed; the MPNN plugin is unavailable"
            )
        ...
        self.seed = seed
        self._module: _MPNNModule | None = None
        self._fitted = False

    def fit(self, X: list[Any], y: np.ndarray) -> TorchMPNN:
        if self._module is None:
            n_out = self.n_classes if self.task_type in ("binary", "multiclass") else 1
            self._module = _MPNNModule(hidden=self.hidden, n_out=n_out, n_layers=self.n_layers)
    ...
    def predict(self, X: list[Any]) -> np.ndarray:
        if not self._fitted or self._module is None:
            raise RuntimeError("TorchMPNN.predict called before fit")
        self._module.eval()
        with torch.no_grad():
            out = self._module(X)
        if self.task_type == "binary":
    ...
    def predict_proba(self, X: list[Any]) -> np.ndarray:
        if not self._fitted or self._module is None:
            raise RuntimeError("TorchMPNN.predict_proba called before fit")
        self._module.eval()
        with torch.no_grad():
...
class MPNNPlugin:
    ...
    def applicability(self, task_type: str, representation_name: str) -> tuple[bool, str]:
        ...
        if representation_name != "graph":
            return False, "MPNN requires the graph representation"
        return True, "CPU-capable message passing network over molecular graphs"
